 ...
 *
 * <p>Caveats to weigh when choosing an expectation:
 *
 * <ul>
 *   <li>it requires sourcing: under {@link com.opencqrs.framework.command.SourcingMode#NONE} no head is exposed, so
 *       {@link AtMost} never rejects and {@link Exactly} always rejects;
 *   <li>under {@link com.opencqrs.framework.command.SourcingMode#RECURSIVE} the head is that of the whole sourced
 *       subject hierarchy;
 *   <li>evaluating an expectation, and the store's write-time
 *       {@link com.opencqrs.esdb.client.Precondition.SubjectIsOnEventId} preconditions the router derives per sourced
 *       subject, only detect concurrent <em>modifications to subjects that already existed</em> when the command
 *       sourced &mdash; a <em>new child subject</em> created concurrently between sourcing and append raises the
 *       hierarchy's effective version yet is guarded by neither (the store has no recursive
 *       {@code SubjectIsOnEventIdRecursively} precondition to reject it), so cross-subtree, set-based invariants (e.g.
 *       uniqueness across children) <strong>cannot</strong> be enforced through an expectation under {@code RECURSIVE}.
 * </ul>
 *
